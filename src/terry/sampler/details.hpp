...
struct add_dst_mul_src_channel
{
	...
	{
		//dst += channel_convert<DstChannel>( src * _w );
...
struct process1Dresampling
{
	void operator( )( const std::vector<SrcP>& src, const std::vector<F>& weight, DstP& dst ) const
	{
	   ...
	   {
		   DstP srcConv;
		   color_convert( src.at(i), srcConv );
		   details::add_dst_mul_src< DstP, F, DstP > ( )( srcConv, weight.at(i) , mp );
	   }
	   dst = mp;

		//DstP mp( 0 );
		//for( std::size_t i = 0; i < src.size(); i++ )
		//	details::add_dst_mul_src< DstP, F, DstP > ( )( color_convert<DstP>(src.at(i)), weight.at(i) , mp );//details::add_dst_mul_src<SrcP, F, DstP > ( )( src.at(i), weight.at(i) , mp );
		//dst = mp;
	}
};
