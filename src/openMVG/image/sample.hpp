...
struct Sampler2d
{
   ...
   **/
  template <typename T>
  T operator()( const cv::Mat & src , const float y , const float x ) const
  {
    const int im_width = src.cols;//src.Width() ;
    const int im_height = src.rows;//Height() ;
    ...
    for( int i = 0 ; i < SamplerFunc::neighbor_width ; ++i )
    {
      ...
      for( int j = 0 ; j < SamplerFunc::neighbor_width ; ++j )
      {
        ...
        const typename RealPixel<T>::real_type pix = RealPixel<T>::convert_to_real( src.at<uchar>( cur_i , cur_j ) ) ;
