...
public:
	IMarker();
	IMarker( const rom::Point2dN<double> & centerImg ): _centerImg( centerImg ) {}
	 ...
	 */
	void setCenterImg( const rom::Point2dN<double>& center ) { _centerImg = center; }

	/** @brief Set center position in the scene
	 * @param center center position
	 */
	void setCenterScene( const rom::Point3d<double>& center ) { _centerScene = center; }
	 ...
	 */
	const rom::Point2dN<double>& centerImg() const { return _centerImg; }

	/** @brief get center inside 2D image
	 * @return center 2D position
	 */
	rom::Point2dN<double>& centerImg() { return _centerImg; }

	/** @brief get center inside 3D scene
	 * @return center 3D position
	 */
	const rom::Point3d<double>& centerScene() const { return _centerScene; }
...
protected:
	rom::Point2dN<double> _centerImg;     ///< Center of the marker inside original image
	rom::Point3d<double> _centerScene;   ///< Center of the marker inside scene @todo REMOVE THIS !
