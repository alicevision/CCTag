...
boost::numeric::ublas::matrix<typename MatC::value_type> linesFromRank2Conic( const MatC & c )
{
	...
	diagonal_matrix<T> s;
	cctag::numerical::svds( c, u, v, s, 2 );
	for( std::size_t k = 0; k < s.size1(); ++k )
	{
...
void signature( const MatA & x, std::size_t & s1, std::size_t & s2, const double epsilon = 10e-8 )
{
	...
	ublas::matrix<T> xn = x / cctag::numerical::norm2( x );
	ublas::diagonal_matrix<T> d;
	cctag::numerical::eigd( xn, d );
...
void rectifyHomography2PlanarCC(const cctag::numerical::geometry::Ellipse & e11, const cctag::numerical::geometry::Ellipse & e21, boost::numeric::ublas::bounded_matrix<double, 3, 3> & mH, const double distMarkers = 1.0);
