    ...
    namespace vision {
        namespace marker {
            namespace cctag {
                ...
                //// TODO template la fonction unique et la déplacer dans une toolbox
                //inline std::vector<EdgePoint*> unique(std::vector<EdgePoint*> & v)
                //{
                //	std::sort(v.begin(),v.end());
                //	std::vector<EdgePoint*>::iterator it;
                //	it = std::unique (v.begin(), v.end());
                //	v.resize( it - v.begin() );
                //}

                bool initMarkerCenter(rom::Point2dN<double> & markerCenter, const std::vector< std::vector< Point2dN<double> > > & markerPoints, int realPixelPerimeter) {
                    ...
                    try {
                        if (realPixelPerimeter > 200) {
                            if (markerPoints[0].size() > 20) {
                                //numerical::ellipseFitting<std::vector< std::vector< Point2dN<double> > >, Point2dN<double>, false>( innerEllipse, markerPoints[0] );
                ...
                bool addCandidateFlowtoCCTag(const std::vector< EdgePoint* > & filteredChildrens, const std::vector< EdgePoint* > & outerEllipsePoints, const rom::numerical::geometry::Ellipse& outerEllipse, std::vector< std::vector< Point2dN<double> > >& cctagPoints, std::size_t numCircles) {
                    ...
                    //ROM_COUT("Center=[" << outerEllipse.center().x() << ";" << outerEllipse.center().y() << "];" );
                    ...
                    for (std::vector<EdgePoint*>::const_iterator it = filteredChildrens.begin(); it != filteredChildrens.end(); ++it) {
                        ...
                        //ROM_COUT(" y = polyval([ " << -lineThroughCenter(0)/lineThroughCenter(1) << ";" << -lineThroughCenter(2)/lineThroughCenter(1) << "],x);");
                        //ROM_COUT("plot(x,y,'y');");
                        ...
                        for (std::size_t j = 1; j < numCircles; ++j) {
                            ...
                            if (!p->_processedAux) {
                                ...
                                if (isInEllipse(outerEllipse, pointToAdd) && isOnTheSameSide(outerPoint, pointToAdd, lineThroughCenter)) { // isInHull( innerBoundEllipse, outerEllipse, pMid ) && isInHull( innerBoundEllipse, outerEllipse, pointToAdd ) &&
                ...
                numerical::geometry::Cercle computeCircleFromOuterEllipsePoints(const std::vector<EdgePoint*>& filteredChildrens, const Point2dN<int> & p1, const Point2dN<int> & p2) {
                    ...
                    double distMax = std::min(rom::numerical::distancePoints2D((Point2dN<int>)(*pMax), p1), rom::numerical::distancePoints2D((Point2dN<int>)(*pMax), p2));

                    double dist;

                    BOOST_FOREACH(const EdgePoint * const e, filteredChildrens) {
                        dist = std::min(rom::numerical::distancePoints2D((Point2dN<int>)(*e), p1), rom::numerical::distancePoints2D((Point2dN<int>)(*e), p2));
                    ...
                    //ROM_COUT_VAR(resCircle);

                    //	{
                    //		double theta = q.angle() * 180.0 / boost::math::constants::pi<double>();
                    //		cvEllipse(simg, cvPoint((int)boost::math::round(q.center().x()), (int)boost::math::round(q.center().y())), cvSize((int)q.a(), (int)q.b()), 360.0 - theta, 0, 360, CV_RGB(0, 0, 255) , 1, 8, 0);
                    //
                    //		cvShowImage("Ellipse",simg);
                    //		cvWaitKey(0);
                    //
                    //		std::cout << std::endl;
                    //		std::cout << "mauvais conditionnement => cercle " << std::endl;
                    //		std::cout << std::endl;
                    //	}
                    return resCircle;
                }
                ...
                bool ellipseGrowingInit(std::vector<EdgePoint> & points, const std::vector<EdgePoint*>& filteredChildrens, numerical::geometry::Ellipse& ellipse) {
                    ...
                    if (isGoodEGPoints(filteredChildrens, p1, p2)) {
                        // compute the ellipse which best fits filteredChildrens
                        // TODO Ellipse(pts) doit être un constructeur de Ellipse !!!!!!!!
                        numerical::ellipseFitting(ellipse, filteredChildrens);
                        //ROM_COUT_DEBUG("Not Create circle");
                    } else {

                        // Initialize ellipse to a circle if there are no points enough
                    ...
                    // todo@Lilian -- virer cette boucle et modifier l'ellipseGrowing en incrementant le runId.
                ...
                void testEllipseGrowingInit()
                {
                        ...
                        Ellipse q;
                        ellipseGrowingInit(childrens, q);

                        std::cout << "Ellipse : " << q.matrix()/q.matrix()(2,2) << std::endl;
                }
                 */

                void connectedPoint(std::vector<EdgePoint*>& pts, const int runId, const EdgePointsImage& img, numerical::geometry::Ellipse& qIn, numerical::geometry::Ellipse& qOut, int x, int y) // TODO inline ?
                {
                    using namespace boost::numeric::ublas;
                ...
                void computeHull(const numerical::geometry::Ellipse& ellipse, double delta, numerical::geometry::Ellipse& qIn, numerical::geometry::Ellipse& qOut) {
                    qIn = numerical::geometry::Ellipse(rom::Point2dN<double>(ellipse.center().x(), ellipse.center().y()), std::max(ellipse.a() - delta, 0.001), std::max(ellipse.b() - delta, 0.001), ellipse.angle());
                    qOut = numerical::geometry::Ellipse(rom::Point2dN<double>(ellipse.center().x(), ellipse.center().y()), ellipse.a() + delta, ellipse.b() + delta, ellipse.angle());
                }

                void ellipseHull(const EdgePointsImage& img, std::vector<EdgePoint*>& pts, numerical::geometry::Ellipse& ellipse, double delta) {
                ...
                /*
                void testEllipseHull()
                {
                        //	std::vector<EdgePoint* > edgePts;
                        //	std::ifstream fid("data_edges.txt", std::ios::in);  // on ouvre en lecture
                        //
                        //	if(fid)  // si l'ouverture a fonctionné
                        //	{
                        //			std::string containt;  // déclaration d'une chaîne qui contiendra la ligne lue
                        //
                        //			while(getline(fid, containt))
                        //			{
                        //				//std::cout << containt << std::endl;  // on affiche la ligne
                        //
                        //				char * cstr, * p;
                        //
                        ...
                        //				//@todo: eloi says: check if (int)vec(0),(int)vec(1) is okay
                        //				EdgePoint * ePt = new EdgePoint((int)vec(0),(int)vec(1), 1.0f, 1.0f, NULL);
                        //				edgePts.push_back(ePt);
                        //
                        //				delete[] cstr;
                        //
                        //			}
                        ...
                        //	}
                        //
                        //	//boost::multi_array<bool, 2> processed( boost::extents[img.shape()[0]][img.shape()[1]] );
                        //		// fill with zeros
                        //	//	std::fill( processed.data(), processed.data() + processed.num_elements(), 0 );
                        //
                        //
                        //
                        //	boost::multi_array<EdgePoint *, 2> img( boost::extents[1000][1000] );
                        //	// fill with zeros
                        //	std::fill( img.data(), img.data() + img.num_elements(),  (EdgePoint*) NULL  );
                        //
                        //	// Création de la map des points contours
                        //	for(int i=0 ; i<edgePts.size() ; ++i)
                        //	{
                        //		img[edgePts[i]->x()][edgePts[i]->y()] = edgePts[i];
                        //		//std::cout << edgePts[i]->cartesian().vec << std::endl;
                        //	}
                        //
                        //	// Points initiaux avant l'appel à ellipseHull
                        //	std::vector<EdgePoint* > pts;
                        //	std::cout << "Size of the initial edge list : " << edgePts.size() << std::endl;
                        //	std::cout << "Size of the edge list : " << edgePts.size()-15 << std::endl;
                        //	for(int i=0 ; i<edgePts.size()-15 ; ++i)
                        //	{
                        //		pts.push_back(edgePts[i]);
                        //	}
                        //
                        //	bounded_matrix<double, 3, 3> Q(3,3);
                        //
                        //	Q(0,0) = 0.000004843102724 ; Q(0,1) = -0.000000331518148 ; Q(0,2) = -0.002146930873619 ;
                        //	Q(1,0) = -0.000000331518148 ; Q(1,1) = 0.000002162305861 ; Q(1,2) = -0.000188785844572 ;
                        //	Q(2,0) = -0.002146930873619 ; Q(2,1) = -0.000188785844572; Q(2,2) = 1.000000000000000 ;
                        //
                        //	Ellipse q = Ellipse(Q);
                        //	double delta = 2.f;
                        //
                        //	ellipseHull(img, pts, q, delta);
                        //
                        //	std::cout << "Size of the new edge list : " << pts.size() << std::endl;
                ...
                void ellipseGrowing(const EdgePointsImage& img, const std::vector<EdgePoint*>& filteredChildrens,
                    ...
                    /*
                    BOOST_FOREACH(EdgePoint* pt, outerEllipsePoints)
                       {
                     *(simg->imageData + pt->y() * simg->widthStep + 3*pt->x()) = 255;
                     *(simg->imageData + pt->y() * simg->widthStep + 3*pt->x() + 1) = 0;
                     *(simg->imageData + pt->y() * simg->widthStep + 3*pt->x() + 2) = 0;
                       }
                     */
                ...
                void ellipseGrowing2(const EdgePointsImage& img, const std::vector<EdgePoint*>& filteredChildrens,
                    ...
                    //int lastSizePtsCircle = outerEllipsePoints.size();
                    //int newSizePtsCircle = 0;
                    //int maxPtsCircle = lastSizePtsCircle;
                    ...
                    //ROM_COUT("In ellipse growing -- after 1st loop");
                    //coutVPoint(outerEllipsePoints);

                    //ROM_COUT_VAR(lastSizePoints);
                    //ROM_COUT_VAR(outerEllipsePoints.size());
                    //ROM_COUT_VAR(outerEllipsePoints.size() - lastSizePoints);
                ...
                // Test ellipseGrowing
                /*
                void testEllipseGrowing()
                {
                        //	std::vector<EdgePoint* > edgePts;
                        //
                        //	readPointsFromFile("data_edges_eGrowing.txt", edgePts);
                        //	std::cout << "Size of the edge list find with EG matlab version: " << edgePts.size() << std::endl;
                        //
                        //	boost::multi_array<EdgePoint *, 2> img( boost::extents[1000][1000] );
                        //	// fill with zeros
                        //	std::fill( img.data(), img.data() + img.num_elements(),  (EdgePoint*) NULL  );
                        ...
                        //	// Points initiaux avant l'appel à ellipseHull
                        //	std::vector<EdgePoint* > pts;
                        //	readPointsFromFile("data_edges_eGInit.txt", pts);
                        //
                        //	std::cout << "Size of the initial edge list : " << pts.size() << std::endl;
                        //
                        //	bounded_matrix<double, 3, 3> Q(3,3);
                        //
                        //	Q(0,0) = 0.0003355609605 ; Q(0,1) = 0.0002699711042 ; Q(0,2) = -0.2124200949865;
                        //	Q(1,0) = 0.0002699711042 ; Q(1,1) = 0.0006012027365 ; Q(1,2) = -0.3125355217814;
                        //	Q(2,0) = -0.2124200949865 ; Q(2,1) = -0.3125355217814 ; Q(2,2) = 185.7094858591270 ;
                        //
                        //	Ellipse q = Ellipse(Q);
                        //	double delta = 2.f;
                        //
                        ...
                        //
                        //	ellipseGrowing(img, pts, outerEllipsePoints, q);
                        //
                        //	std::cout << "Size of the new edge list : " << outerEllipsePoints.size() << std::endl;
                        //	std::cout << "Q :" << q.matrix() << std::endl;
                        //	//for(int i=0 ; i<pts.size() ; ++i)
                        //	//{
                        //	//		std::cout << pts[i]->cartesian().vec << std::endl;
                        //	//}
                }
                 */

                /*
                rom::vision::Marker markerInitialization(const std::vector<EdgePoint*> & childrens, std::vector<EdgePoint*> & outerEllipsePoints, numerical::geometry::Ellipse & outerEllipse)
                {
                        using namespace rom::vision::detection;
                        // TODO Construire le marqueur à partir des enfants, des points de l'ellipse extérieure et de l'ellipse extérieure. Les points en entrée sont de type EdgePoint * et doivent être passé en
                        // parametre au constructeur sous la forme de bounded_vector<double, 3, 3>
                        return Marker();
                }
                 */
