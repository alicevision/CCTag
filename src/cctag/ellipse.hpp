...
inline bool isInEllipse( const rom::numerical::geometry::Ellipse& ellipse, const Point2dN<double> & p)
{
	using namespace boost::numeric;
    return ( ( ublas::inner_prod( p, ublas::prec_prod( ellipse.matrix(), p ) ) ) * ublas::inner_prod( ellipse.center(), ublas::prec_prod( ellipse.matrix(), ellipse.center() )) > 0 ); //here ( d < 15 );
}

/**
 * Check if ellipses are surimposed
 * @param ellipse1
 * @param ellipse2
 * @return 
 */
inline bool isSameEllipses( const rom::numerical::geometry::Ellipse& ellipse1, const rom::numerical::geometry::Ellipse& ellipse2 )
{
    // ( rom::numerical::distancePoints2D( ellipse1.center(), ellipse2.center() ) < std::min( ellipse1.a(), ellipse1.b() ) );
	return ( isInEllipse( ellipse1, ellipse2.center() ) || isInEllipse( ellipse2, ellipse1.center() ) );//here ( d < 15 );
}
...
//bool addCandidateFlowtoCCTag( const std::vector< std::vector<EdgePoint*> >& vCandidateFlowEdgePointToAdd, const rom::numerical::geometry::Ellipse& outerEllipse, std::vector< std::vector< Point2dN<double> > >& cctagPoints );
...
bool ellipseGrowingInit( std::vector<EdgePoint> & points, const std::vector<EdgePoint*>& filteredChildrens, rom::numerical::geometry::Ellipse& ellipse );
