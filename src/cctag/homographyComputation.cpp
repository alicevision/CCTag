	                               ...
	                               boost::numeric::ublas::matrix<double> & M )
{
	...
	for( std::size_t s = 0; s < 3; ++s )
	{
		...
		std::size_t s1, s2;
		signature( X / rom::numerical::normFro( X ), s1, s2 );
		S( 0, s ) = s1;
		S( 1, s ) = s2;

		ublas::bounded_matrix<double, 3, 3> uu, vv, ss;
		rom::numerical::svds( X, uu, vv, ss, s1 + s2 );

		ublas::matrix<double> q = ublas::prod( ublas::prod< ublas::matrix<double> > ( uu, ss ), ublas::trans( vv ) );
		q = q / rom::numerical::normFro( q );
		degQuad[s] = q / rom::numerical::normFro( q );
		gVector[s] = rom::numerical::normalize( ublas::bounded_vector<double, 3>( ublas::column( null( degQuad[s] ), 0 ) ) );
	}
        ...
        for( std::size_t s = 0; s < 2; ++s )
        {
            ...
            rom::numerical::svds( degQuad[ gvec[ s ] ], L, R, S, 2 );
            ublas::matrix<double> di = ublas::prod( L, rom::numerical::matSqrt( S ) );
